...
String EndOfLine = "\n";
String EndOfField = "\n\ff";    //end of field is also the end of a line
...
public FrameFormat string2Frame (String str){               //transforms msg back to frame
    ...
    for (int i = 1; i < splitByFields.length-1; i++) {
      String headerName = splitByFields[i].split(":")[0];
      String headerValue = splitByFields[i].split(":")[1];
    ...
    //body:
    String sBody = splitByFields[splitByFields.length-1];
